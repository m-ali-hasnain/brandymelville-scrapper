...
import time
from ..items import BrandymelvilleScrapperItem
import re
from webdriver_manager.chrome import ChromeDriverManager

# Constants
SCROLL_PAUSE_TIME = 5 # Scroll pause time for scrapping pages having infinite scroll
ALLOWED_CATEGORIES = ["clothing"] # Allowed Categories for current domain
...
WEBSITE_NAME = "brandymelville"
# This class represents our Spider which crawls BrandyMilville.
class BrandymelvilleScrapper(scrapy.Spider):
    name = 'brandymelville' # name of crawler
    allowed_domains = ['us.brandymelville.com']
    ...
    def start_requests(self):
        base_url = 'https://us.brandymelville.com/collections/clothing'
        yield scrapy.Request(url=base_url, callback=self.parse_categories)



    def parse_categories(self, response):
        categories = response.css("ul.main-navigation li:nth-child(3)>ul.main-navigation-childlinks-list li a::attr('href')").getall()[1:]
        for category in categories:
            meta_category = category.split("/")[-1]
            if re.search("-", meta_category):
                if re.search("sweatpants|sweatshirts", meta_category, re.IGNORECASE):
                    meta_category = "sweatshirts"
                else:
                    meta_category = ' '.join(meta_category.split("-")[1:])
            else:
               meta_category = meta_category

            yield scrapy.Request(url=response.urljoin(category), callback=self.parse_products, meta={"categories": [meta_category]})



    def parse_products(self, response):
        self.driver.get(response.request.url)
        # Now we will implement infinite scrolling technique for parsing all products on current page
        ...
        category_page = Selector(text=self.driver.page_source)
        # Once page is fully loaded scrapping all products
        product_urls = category_page.css("ul#main-collection-product-grid li div a::attr('href')").getall()
        for p_url in product_urls:
            yield scrapy.Request(response.urljoin(p_url), self.parse_product, meta = {"categories": response.meta.get("categories", [])})


    def parse_product(self, response):
        self.driver.get(response.request.url)
        custom_page = Selector(text=self.driver.page_source)
        url = response.request.url
        external_id = response.css("span#skunumber::text").get().strip()
        brand = ""
        product_name = response.css("h1.product__title::text").get().strip()
        name = f"{brand} {product_name}" if len(brand)!=0 else product_name
        categories = []
        ...
        price = response.css("span.price-item.price-item--regular::text").get().strip()
        colors = custom_page.xpath("//div[@class = 'color-option color-option--visible'] /@data-value").getall()
        colors.append(response.xpath("//div[@class='color-option color-option--visible'] /@data-value").get())
        colors = [color for color in colors if color is not None]
        colors = list(set(colors))
        sizes = response.css("input[name='Size']::attr('value')").get().split('/')
        details = response.css("div.product__description.rte *::text").getall()
        details = self.clean_details(details)
        fabric = self.find_fabric_from_details(details)
        images = response.xpath("//ul[@class = 'product-gallery__nav']  /li[@class = 'product-gallery__nav-item  ' or @class = 'product-gallery__nav-item product-gallery__nav-item--active ' ] //modal-opener /div /img /@src").getall()
        if not images:
            images = [response.css(".product__media.media.media--transparent img::attr('src')").get()]

        images = [('https:'+ image.split(',')[0]) for image in images]
        fit = ' '.join(self.find_keywords_from_str(details, name, url, FIT_KEYWORDS)).strip()
        neck_line = ' '.join(self.find_keywords_from_str(details, name, url, NECK_LINE_KEYWORDS)).strip()
        length = ' '.join(self.find_keywords_from_str(details, name, url, LENGTH_KEYWORDS)).strip()
        occasions = self.find_keywords_from_str(details, name, url, OCCASIONS_KEYWORDS)
        style = self.find_keywords_from_str(details, name, url, STYLE_KEYWORDS)
        gender = "women"
        number_of_reviews = ""
        review_description = []
        top_best_seller = ""
        ...
        if re.search("Measurement", details):
            details = details.split("Measurement")[:-1]
        else:
            details = details.split("Measurements")[:-1]

        # Now Creating Item
        item = BrandymelvilleScrapperItem()
        item["url"] = url
    ...
    def clean_details(self, details):
        ...
        details = [detail for detail in details if detail!="" and detail!="\n"]
        return details
    ...
    # This function remove trailing spaces if any
    def remove_trailing_spaces_if_any(self, str):
